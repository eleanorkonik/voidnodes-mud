...
- **Flawless recruit also reveals first backstory aspect** (implemented) — visit every tile with zero eliminations. Uses same `npc.backstory.aspects[]` data and appends to `npc.aspects.other[]`. Tracks via `npc.revealed_backstory[]`. CONFIDE should check `revealed_backstory` to skip already-revealed aspects.
