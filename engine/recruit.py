...
# Color-specific step flavor (keyed by color char)
COLOR_STEP_FLAVOR = {
...
def apply_move(state, direction):
    ...
    for color in counters:
        if counters[color] == 2 and color not in newly_eliminated_colors:
            color_info = next((c for c in RECRUIT_COLORS if c[0] == color), None)
            if color_info:
                messages.append(f"{display.BRIGHT_RED}{rng.choice(WARN_FLAVOR)}{display.RESET}")
    ...
    for color in newly_eliminated_colors:
        ...
        messages.append(f"{display.RED}{rng.choice(ELIM_FLAVOR)}{display.RESET}")
...
def get_step_flavor(progress_ratio, rng=None):
    """Pick a progress-appropriate flavor line."""
    rng = rng or random
    if progress_ratio < 0.35:
        return rng.choice(STEP_FLAVOR_EARLY)
    elif progress_ratio < 0.75:
        return rng.choice(STEP_FLAVOR_MID)
    else:
        return rng.choice(STEP_FLAVOR_LATE)


def get_npc_flavor(npc, progress_ratio):
    """Get NPC-specific atmosphere text based on progress."""
    name = npc.get("name", "They")
    if progress_ratio < 0.25:
        return f"{name} watches you carefully, arms crossed."
    elif progress_ratio < 0.5:
        return f"{name} is listening, but hasn't committed to anything yet."
    elif progress_ratio < 0.75:
        return f"{name} is leaning in, listening closely."
    else:
        return f"{name} is nodding slowly. You're getting through."
